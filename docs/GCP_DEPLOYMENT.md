...
Deploy PocketBase to Google Cloud for master-chef.
...
gcloud auth login
gcloud projects create master-chef-backend --name="Master Chef"
gcloud config set project master-chef-backend
...
```bash
# Build and deploy
gcloud builds submit --tag gcr.io/master-chef-backend/pocketbase
gcloud run deploy pocketbase \
  --image gcr.io/master-chef-backend/pocketbase \
  --platform managed \
  --region us-central1 \
...
3. Create collections (recipes, scores)
4. Set API rules
